...
Go REST API implementing the Catalog bounded context (library-docs/02-domain/domain-map.md):
owns the book catalog and tracks copy availability. Hexagonal Architecture — see
`library-docs/05-architecture/decisions/records/ADR-002-hexagonal-modular-monolith.md`.

Part of the microservices split: this service owns the `books` table exclusively (its own
PostgreSQL database, `catalog_db`). It validates the JWT issued by access-service using the
...
├── application/usecase/  → CreateBook (HU-04), SearchBooks (HU-05)
├── config/                → environment variable loading
└── infrastructure/
    ├── http/               → chi router, middleware, handlers (primary adapters)
    ├── postgres/            → BookRepository (secondary adapter)
