...
a mid-sequence failure (e.g. the loan fails to save after the copy was already
decremented) is an accepted risk at this project's size.
...
├── application/usecase/  → RegisterLoan (HU-06)
...
`MEMBERSHIP_SERVICE_URL` / `CATALOG_SERVICE_URL` to register a loan.
