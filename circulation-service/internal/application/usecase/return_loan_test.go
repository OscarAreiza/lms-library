...
// RegisterLoan flow, which lives on a separate branch/usecase) so HU-07's
// ReturnRegistrationService has something to return.
func setupLoanFixture(t *testing.T) (*service.ReturnRegistrationService, *circulation.Loan, *fakeStudentClient, *fakeBookClient) {
	t.Helper()
	...
	return svc, loan, students, books
}

func TestReturnLoan_OnTime(t *testing.T) {
	svc, loan, _, books := setupLoanFixture(t)
...
func TestReturnLoan_Late_SuspendsStudent(t *testing.T) {
	svc, loan, students, _ := setupLoanFixture(t)
...
func TestReturnLoan_RejectsDoubleReturn(t *testing.T) {
	svc, loan, _, _ := setupLoanFixture(t)
	uc := usecase.NewReturnLoan(svc)
