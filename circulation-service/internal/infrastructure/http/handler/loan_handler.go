...
// LoanHandler implements the /loans endpoints relevant to HU-08: Return
// (which triggers the suspension policy when late) and the Overdue report.
type LoanHandler struct {
	returnLoan   *usecase.ReturnLoan
	overdueLoans *usecase.OverdueLoans
}

func NewLoanHandler(returnLoan *usecase.ReturnLoan, overdueLoans *usecase.OverdueLoans) *LoanHandler {
	return &LoanHandler{returnLoan: returnLoan, overdueLoans: overdueLoans}
}
...
// Return — POST /loans/{id}/return (HU-07; triggers HU-08's suspension policy
// when the return is late — FR-019).
