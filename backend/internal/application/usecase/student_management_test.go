...
// fakeLoanRepository — Fake test double (11-quality/tdd-guide.md). Only the method
// DeactivateStudent actually needs is exercised in these tests.
type fakeLoanRepository struct {
	activeCountByStudent map[string]int
}

func (f *fakeLoanRepository) FindByID(_ context.Context, _ string) (*circulation.Loan, error) {
	return nil, circulation.ErrLoanNotFound
}

func (f *fakeLoanRepository) CountActiveByStudent(_ context.Context, studentID string) (int, error) {
	return f.activeCountByStudent[studentID], nil
}

func (f *fakeLoanRepository) Search(_ context.Context, _ string, _ bool, _, _ string, _, _ int) ([]*circulation.Loan, int, error) {
	return nil, 0, nil
}

func (f *fakeLoanRepository) Save(_ context.Context, _ *circulation.Loan) error {
	return nil
}
...
func TestDeactivateStudent_RejectsWhenHasActiveLoans(t *testing.T) {
	...
	repo := newFakeStudentRepository()
	student := seedFakeStudent(t, repo)
	loans := &fakeLoanRepository{activeCountByStudent: map[string]int{student.ID: 1}}
...
func TestDeactivateStudent_SucceedsWithNoActiveLoans(t *testing.T) {
	repo := newFakeStudentRepository()
	student := seedFakeStudent(t, repo)
	loans := &fakeLoanRepository{activeCountByStudent: map[string]int{}}
	uc := usecase.NewDeactivateStudent(repo, loans)
